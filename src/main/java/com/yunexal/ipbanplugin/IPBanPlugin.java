...
public class IPBanPlugin {
    ...
    @Inject
    public IPBanPlugin(ProxyServer server, Logger logger, @DataDirectory Path dataDirectory) {
        this.server = server;
        this.logger = logger;
    ...
    public void onProxyInitialization(ProxyInitializeEvent event) {
        logger.info("IPBanPlugin is starting...");

        // Load config and messages
        configManager.loadConfig();
        messageManager.loadMessages();

        // Initialize database (this will also load libraries)
        ...
        logger.info("IPBanPlugin has been enabled!");
    }

    @Subscribe
    public void onProxyShutdown(ProxyShutdownEvent event) {
        databaseManager.close();
        logger.info("IPBanPlugin has been disabled!");
    }
}
